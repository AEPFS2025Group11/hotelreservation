...
from app.services.facility_service import FacilityService, get_facility_service
...
async def get_facilities(service: FacilityService = Depends(get_facility_service)):
    logger.info("GET /api/facilities - Fetching all facilities")
    return service.get_all()


@router.get("/{facility_id}", response_model=FacilityOut)
async def get_facility_by_id(facility_id: int, service: FacilityService = Depends(get_facility_service)):
...
async def create_facility(data: FacilityIn, service: FacilityService = Depends(get_facility_service)):
    logger.info("POST /api/facilities - Creating new facility")
    return service.create(data)


@router.put("/{facility_id}", response_model=FacilityOut)
async def update_facility(facility_id: int, data: FacilityUpdate,
...
async def delete_facility(facility_id: int, service: FacilityService = Depends(get_facility_service)):
